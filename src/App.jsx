    ...
    <div>
        ...
        <div>
          {latest&&(
            <div style={{background:latest.net>=0?"#f0fdf4":"#fef2f2",border:"1.5px solid "+(latest.net>=0?"#bbf7d0":"#fecaca"),borderRadius:14,padding:"16px 20px",marginBottom:12,textAlign:"center"}}>
              ...
              <div style={{fontSize:30,fontWeight:800,color:latest.net>=0?"#15803d":"#dc2626",marginTop:2}}>{fmtRs(Math.abs(latest.net))}</div>
          ...
          {months.length===0?<Card><div style={{color:"#aaa",fontSize:13,textAlign:"center",padding:"20px 0"}}>No data yet.</div></Card>:months.map(r=>{
            ...
            return (
              <Card key={r.month} style={{marginBottom:10}}>
                <div onClick={()=>setOpenMonth(open?null:r.month)} style={{display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}}>
                  <div style={{minWidth:0}}>
                    <div style={{fontWeight:700,fontSize:14,color:"#1a1a1a"}}>{moLabel(r.month)}</div>
                    <div style={{fontSize:11.5,color:"#94a3b8",marginTop:2}}>Revenue {fmtRs(r.revenue)} · Opex {fmtRs(r.opexTotal)}</div>
                  </div>
                  <div style={{textAlign:"right",flexShrink:0}}>
                    <div style={{fontSize:16,fontWeight:800,color:r.net>=0?"#15803d":"#dc2626"}}>{r.net>=0?"+":""}{fmtRs(r.net)}</div>
                    <div style={{fontSize:12,color:"#94a3b8"}}>{open?"▲":"▼"}</div>
                  </div>
                </div>
                {open&&(
                  <div style={{marginTop:10,borderTop:"1px solid #f1f5f9",paddingTop:8}}>
                    <div style={{fontSize:11,fontWeight:700,color:"#94a3b8",textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:6}}>Opex breakdown</div>
                    {subs.length===0?<div style={{fontSize:12,color:"#aaa"}}>No Opex transactions this month.</div>:subs.map(sx=>(
                      <div key={sx.name} style={{display:"flex",justifyContent:"space-between",fontSize:12.5,padding:"3px 0",color:"#64748b"}}><span>{sx.name}</span><span style={{fontWeight:600,color:"#1a1a1a"}}>{fmtRs(sx.val)}</span></div>
                    ))}
      ...
      {view==="cattle"&&(
        <Card>
          ...
          <div style={{fontSize:11,color:"#94a3b8",marginTop:12}}>Feed-level efficiency only (excludes overheads). Dry cattle show a loss as they produce no milk.</div>
        </Card>
      )}
    </div>
  );
